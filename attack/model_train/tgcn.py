...
class tgcnCell(RNNCell):
    ...
    def __init__(self, num_units, tmp_adj, out_keep_prob, num_nodes, input_size=None,
                 act=tf.nn.relu, reuse=None):
    ...
    def _gc(self, inputs, state, output_size, bias=0.0, scope=None):   # graph convolution,
        ...
        with tf.variable_scope(scope):
            ...
            x = tf.reshape(x, shape=[-1, self._nodes * output_size])
        return tf.nn.dropout(x, self.out_keep_prob)
